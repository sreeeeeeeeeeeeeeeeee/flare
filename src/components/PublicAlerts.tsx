...
const PublicAlerts = ({ alerts, className }: PublicAlertsProps) => {
  const [filter, setFilter] = useState<'all' | 'critical' | 'warning' | 'info'>('all');
  
  // Filter alerts that are marked as public visibility
  const publicAlerts = alerts.filter(alert => 
    alert.visibility === 'public' || alert.visibility === undefined
  );
    ...
    <div className={cn("border border-border rounded-lg bg-card flex flex-col overflow-hidden", className)}>
      <div className="p-3 border-b border-border flex justify-between items-center">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-medium text-sm">Public Alerts</h3>
        </div>
